...
//import com.data.config.CorsFilter;
import com.data.bean.DynamicCellBean;

import com.data.bean.ExportHeaderBean;
...
//    public static final Logger LOGGER = LoggerFactory.getLogger(CorsFilter.class);
    ...
    public File exportFile(String exportFile, int lastColumn, HttpServletRequest req)
            throws Exception {
        String prefixOutPutFile = new SimpleDateFormat("yyyyMMddHHmmss_").format(new Date()) + CommonUtil.getUserLoginName(req) + "_";
     ...
     * @throws Exception Exception
     */
    public String exportFile(String exportFile, HttpServletRequest req) throws Exception {
        String prefixOutPutFile = new SimpleDateFormat("yyyyMMddHHmmss_").format(new Date()) + CommonUtil.getUserLoginName(req) + "_";
        exportFile = prefixOutPutFile + exportFile;
        String fullPathFile;
        if (isXLSX) {
            fullPathFile = Constants.COMMON.EXPORT_FOLDER + exportFile + ".xlsx";
            view.writeXLSX(fullPathFile);
        } else {
            fullPathFile = Constants.COMMON.EXPORT_FOLDER + exportFile + ".xls";
            view.write(fullPathFile);
        }
        return fullPathFile;
    }
...
//    }
    ...
    public void exportPdfFile(String exportFile, HttpServletRequest req)
            throws Exception {
        exportFile = Constants.COMMON.EXPORT_FOLDER + exportFile + CommonUtil.getUserLoginName(req) + ".pdf";
...
//            LOGGER.debug("debug", e);
    ...
    public void addText(String str, int row, int col) throws CellException {
        ...
        view.setText(row, col, CommonUtil.NVL(text) + CommonUtil.NVL(str));
    }
    //#042 Start
    /**
     * clearCell
